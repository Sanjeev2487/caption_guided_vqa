...
class TopDownModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.w_emb.init_embedding('h5data/glove6b_init_300d.npy')

        self.caption_w_emb = WordEmbedding(self.vocab_size, emb_dim=300, dropout=0.4)
        self.caption_w_emb.init_embedding('glove6b_caption_init_300d.npy')
        self.caption_emb = CaptionQuestionImageRNN0(c_dim=300, num_hid=640 ,q_dim = 1280,  nlayers=1, bidirect=False, dropout=0.2, rnn_type='GRU')
        self.c_net = FCNet([640, 1280], dropout=0.1, norm='weight', act='LeakyReLU')
    ...
    def _forward(self, v, q, c, rc, with_gt_caption = 1, task = 'VQA'):
        ...
        att = att_1 + att_2
        att_feats = att * v + v
        v_emb_ = (att_feats - v).sum(1)  # [batch, v_dim]

        if with_gt_caption:
            rc = rc.view(-1, rc.size(-1))
            cw_emb = self.caption_w_emb(rc)  # get word embeddings
        else:
            ...
            cw_emb = self.caption_w_emb(seqs.long())  # get word embeddings


        c_emb = self.caption_emb(q_emb, cw_emb, v_emb_)  # run GRU on word embeddings [batch, q_dim]
        ...
        att_3 = att_3.view(q_emb.size(0), n_c, 36, -1).max(1)[0]
        att_4 = att_4.view(q_emb.size(0), n_c, 36, -1).max(1)[0]

        # att = (att_1 + att_2)*(1 + att_3 + att_4)
        v_emb = ((att_feats - v)*(1 + att_3 + att_4)).sum(1) # [batch, v_dim]
        q_repr = self.q_net(q_emb)
        v_repr = self.v_net(v_emb)
        c_repr = self.c_net(c_emb)
        c_repr, _ = c_repr.view(q_repr.size(0), 5, -1).max(1)
        joint_repr = q_repr * (c_repr + v_repr)
    ...
    def _sample(self, v, q, c, rc):
        ...
        seq = v.new_zeros((5 * batch_size, self.seq_length), dtype=torch.long)
        seqLogprobs = v.new_zeros(5 * batch_size, self.seq_length)
        
        for t in range(self.seq_length + 1):
            if t == 0: # input <bos>
                it = p_fc_feats.new_zeros(5 * batch_size, dtype=torch.long)
    ...
    def _sample_beam(self, v, q, c, rc):
        ...
        for k in range(batch_size):
            ...
            tmp_fc_feats = p_fc_feats[5*k:5*k + 1].expand(beam_size, p_fc_feats.size(1))
            tmp_att_feats = p_att_feats[5*k:5*k + 1].expand(*((beam_size,) + p_att_feats.size()[1:])).contiguous()
            tmp_p_att_feats = pp_att_feats[5*k:5*k + 1].expand(*((beam_size,) + pp_att_feats.size()[1:])).contiguous()
            ...
            for t in range(1):
                if t == 0:  # input <bos>
                    it = v.new_zeros([beam_size], dtype=torch.long)
